...
| 7 | [ACL-gated SSH](07-acl-gated-ssh.md) | **Done, W6–W7** (GitHub issue [#108](https://github.com/karst-net/karst/issues/108)). `policy.go`'s `Document.Ssh`/`SshRule`/`CompileSSH` compile a second, independent admission gate reusing `Filter`/`normalize`, with its own nil-vs-empty (absent vs. deny-all, §3.2) and dst-port-suffix validation. `karst_control.proto` fields 18 (`ssh_filter`, reusing `KarstFilterRule`) and 19 (`ssh_filter_present`) carry it over the wire, hashed into `NetmapVersion`/`netmap_version` behind their own `karst-ssh-filter` separator in both languages and pinned by regenerated `spec/vectors/karst-control-v1.json` vectors. `karstd`'s `filter::SshFilter` and a new per-peer `flow::SshAdmissions` cache enforce it once per new flow to port 22 in `Engine::permit` — deliberately *not* cleared on reconfigure, unlike the general ACL's own flow cache, so a revocation reaches only the next connection attempt rather than tearing down one already open (§3.4), pinned by `tests/ssh_gate.rs`. `karst status`/`bugreport` gain a third `[ssh_policy]` section alongside `[policy]`. The console's `validate`/`preview` describe `ssh` grants by principal/target instead of folding them into the acls flow diff, staying in the raw-document editor per §3.5. `go test ./management/internals/karst/{policy,control,api}/...`, `cargo test -p karstd -p karst-control-client`, and the console's typecheck/lint/unit suite all pass. A real two-node namespace exit demonstration (`bins/karstd/tests/aquifer.rs::an_ssh_policy_denies_what_the_acl_alone_would_permit`, `#[ignore]`d like every other row in that file) is written and compiles, but needs `CAP_NET_ADMIN`/root this session did not have standing authorization to use — run it via `just test-aquifer` to confirm end to end before relying on it | Go 2 | W6–W7 |
