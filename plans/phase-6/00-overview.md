...
| 5 | TURN fallback | **Started, W1: server-side credential minting done.** `KarstTurnServer` (`karst_control.proto` field 16 on `KarstNetmapResponse`), the `turncred` package (registry load/validate against `turn:`/`turns:` URIs, HMAC-SHA1 TURN-REST minting via the existing `hmac.TimedHMAC`, `psk.Key`-style redacted `Credential`), wired into `NetmapHandler`/`bootstrap.Install` and `karst-control` via `KARST_TURN_REGISTRY_FILE`/`KARST_TURN_SHARED_SECRET_FILE`/`KARST_TURN_CREDENTIAL_TTL` — opt-in, byte-identical netmaps for a deployment that hasn't configured it. **Still open, tracked in [GitHub issue #92](https://github.com/karst-net/karst/issues/92):** the Rust client (allocation, permissions, channel binding, credential refresh — decided to use `webrtc-rs`'s `turn`/`stun` crates rather than hand-rolling RFC 8656), `coturn` as a 14th `aquifer.rs` topology plus CI wiring, DB-backed admin CRUD + console surface for the TURN registry, a `coturn:` compose service, and a decision on NetBird's parallel legacy `TURNConfig`/`token_mgr.go` credential path. Arrives with the co-located relay path already automatic and lossless (thirteen `karstd` topologies), so this buys ADR-0008 interoperability, not connectivity | Rust 2 + Go 2 | W3–W6 |
