...
| 3 | Internal cryptographic review | Structured self-review of PHREATIC against `spec/phreatic-v1.md`, the Verifpal/ProVerif models, and the vector suite, written up with the [GitHub issue tracker](https://github.com/karst-net/karst/issues?q=is%3Aissue)'s existing discipline. Must start *after* #1 and #2 land — reviewing before the newest signing tier and the newest suite dispatch exist means reviewing a system that will have changed under the review. **Started 2026-09-02, first pass done** — [`phreatic-review-findings.md`](../../phreatic-review-findings.md), three findings so far, two now closed: §9.1's cookie mechanism, built but never wired into `karstd` (GitHub issue [#76](https://github.com/karst-net/karst/issues/76), **closed 2026-09-02** — `Engine` now holds a rotating `CookieSecret`, checks `mac2` when `mac1` fails, and answers an over-threshold fragment with a real `CookieReply`; the initiator side decrypts it and retries once under `mac2`; spec gap filled at §13.10; covered end to end in `bins/karstd/tests/cookie.rs`), the formal models never extended to cover suite `0x0002` (GitHub issue [#78](https://github.com/karst-net/karst/issues/78), **closed 2026-09-02** — `spec/models/phreatic-nodh.vp`, 6/6 under Verifpal, wired into `just verify` and CI), and §7.3's PSK epoch grace period, unimplemented and unimplementable against the current single-PSK data model (GitHub issue [#77](https://github.com/karst-net/karst/issues/77), **still open, next**). Remaining: #77's implementation, a ProVerif equivalent of `phreatic-nodh.vp` (spec §14 item 2's half of the same gap, still open), `karst-crypto` primitive-level reading, §14 item 10's adversarial reading of §13.8, item 9's rekey transition table | Crypto + a second reader (Rust 1, per §4) | W3–W4 |
