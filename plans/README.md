...
- #121 (path MTU discovery) is implemented — ADR-0019, `spec/aven-v1.md` §7.9/§8.4 — superseding PLAN.md §3.3's "Phase 6" deferral and `aven-v1.md`'s former §12 item 5. The ADR records one still-open integration gap: `bins/karstd` never registers a relay path with `PathSet` (§8 rule 2's own comparison is unwired the same way), so the new selection rule is correct and tested but does not yet change live datapath routing.
