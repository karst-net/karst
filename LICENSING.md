...
Every source file **we write** carries an SPDX identifier, and `just
licenses-check` enforces it. When the file header and this table disagree, the
file header wins. The same table appears in [LICENSE](LICENSE), which is the
file GitHub and most tooling read.

**The vendored NetBird fork is the exception.** The 586 upstream files under
`server/` carry no SPDX header and are not given one: they are BSD-3-Clause and
somebody else's copyright, so stamping this project's identifier on them would
be a false claim, and editing them would forfeit the zero-diff property that
makes a security cherry-pick a merge rather than a rewrite
([ADR-0009](docs/adr/0009-control-plane-fork-vs-greenfield.md)). What BSD-3
requires is that its notice travels with the code, and that is
`server/LICENSE-NETBIRD-BSD-3` plus `server/LICENSES/`. Karst's own server code
is all under a `karst` path, which is what the check looks at.
