...
func TestNewNetwork(t *testing.T) {
	...
	assert.False(t, subnetContainsKarstDNSStub(network.Net))
}

func TestAllocatePeerIPReservesKarstDNSStub(t *testing.T) {
	prefix := netip.MustParsePrefix("100.100.0.0/16")
	stub := karstDNSStubAddress
	for i := 0; i < 100; i++ {
		ip, err := AllocatePeerIP(prefix, []netip.Addr{stub})
		require.NoError(t, err)
		assert.NotEqual(t, stub, ip)
	}
}
