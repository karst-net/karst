...
// karstDNSStubAddress is owned by every node's local resolver. It must never
// be handed to a peer, or that peer would shadow the stub for its own account.
var karstDNSStubAddress = netip.MustParseAddr("100.100.100.100")
...
func NewNetwork() *Network {
	...
	r := rand.New(s)
	intn := r.Intn(len(sub))
	// Keeping an entire /16 out of allocation is intentionally coarser than a
	// single reserved host. A hole inside an account's otherwise contiguous
	// prefix becomes a trap for every feature that later iterates that prefix.
	for subnetContainsKarstDNSStub(sub[intn].IPNet) {
		intn = r.Intn(len(sub))
	}
...
func AllocatePeerIP(prefix netip.Prefix, takenIps []netip.Addr) (netip.Addr, error) {
	...
	if prefix.Contains(karstDNSStubAddress) {
		stub := karstDNSStubAddress.As4()
		taken[binary.BigEndian.Uint32(stub[:])] = struct{}{}
	}
...
func AllocateRandomPeerIP(prefix netip.Prefix) (netip.Addr, error) {
	...
	for {
		offset := uint32(rng.Intn(int(totalIPs-2))) + 1
		candidate := uint32ToIP(baseIP + offset)
		if candidate != karstDNSStubAddress {
			return candidate, nil
		}
	}
}

func subnetContainsKarstDNSStub(subnet net.IPNet) bool {
	return subnet.Contains(net.IP(karstDNSStubAddress.AsSlice()))
}
