...
type versionCase struct {
	...
	// SSHFilter is the independent SSH admission gate (plans/phase-6/
	// 07-acl-gated-ssh.md §3.1). SSHFilterPresent carries the nil-vs-empty
	// distinction the wire's ssh_filter_present flag exists for: an absent
	// "ssh" block and one present but granting nothing must move the version
	// differently even when both currently produce an empty rule list.
	SSHFilter        []versionRule  `json:"ssh_filter,omitempty"`
	SSHFilterPresent bool           `json:"ssh_filter_present,omitempty"`
...
func TestVectors(t *testing.T) {
	got := vectorFile{
		...
		Note: "Cross-implementation vectors. Covers derivation and framing, " +
			...
			"incompatible. " +
			"2026-09-10: netmap_version gained the domain-separated karst-ssh-filter " +
			"term for the independent SSH admission gate (plans/phase-6/" +
			"07-acl-gated-ssh.md §3.1), including its presence flag. Pre-change " +
			"version values are intentionally incompatible. " +
	...
	for _, tc := range []struct {
		...
		{
			// An SSH-only policy change, so the vector proves the
			// karst-ssh-filter separator prevents aliasing with the
			// egress-filter byte stream, the same property the previous
			// case proves for egress against ingress.
			name: "the same netmap with an ssh filter",
			resp: &proto.KarstNetmapResponse{
				PskEpoch:  9,
				NodeId:    []byte("node-one"),
				DnsName:   "alpha",
				...
				SshFilter: []*proto.KarstFilterRule{{
					Srcs:  []string{"node-two"},
					Ports: []*proto.KarstPortRange{{First: 22, Last: 22}},
				}},
				SshFilterPresent: true,
			},
			psks: []string{hex.EncodeToString(pattern(32, 0x70))},
		},
		{
			// ssh_filter_present alone, with an empty rule list, must
			// produce a version different from "a node alone in its
			// network" (flag false, no rules): an absent "ssh" block and
			// a present-but-empty one are observably different states
			// (§3.2) and must not collide just because both currently
			// hash the same empty rule list.
			name: "an ssh filter present but empty",
			resp: &proto.KarstNetmapResponse{
				PskEpoch:         9,
				NodeId:           []byte("node-one"),
				DnsName:          "alpha",
		...
		for _, r := range tc.resp.GetSshFilter() {
			vc.SSHFilter = append(vc.SSHFilter, versionRule{
				Srcs:  r.GetSrcs(),
				Ports: portsOf(r.GetPorts()),
			})
		}
		vc.SSHFilterPresent = tc.resp.GetSshFilterPresent()
