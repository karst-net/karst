...
func (p scanPermissions) ValidateDomainScopedPermission(ctx context.Context, accountID, userID, _ string, module modules.Module, operation operations.Operation) (bool, context.Context, error) {
	return p.ValidateUserPermissions(ctx, accountID, userID, module, operation)
}
...
func TestListNodes_OnlyEnrolledNodesAndNoKeyMaterial(t *testing.T) {
	...
	}, nil, nil, nil, nil, nil, nil, nil, nil, scanPermissions{role: types.UserRoleOwner}, nil, router)
...
func TestNodeResponsesNeverContainSecretFixtureMaterial(t *testing.T) {
	...
	router := mux.NewRouter()
	RegisterEndpoints(fakeNodes{"handle-a": {Handle: "handle-a", PublicKey: []byte(secrets[0]), KemPublicKey: []byte(secrets[1])}}, fakePeers{{Key: "handle-a", Name: "node", UserID: "user-a", SSHKey: secrets[2]}}, nil, nil, nil, nil, nil, nil, nil, nil, scanPermissions{role: types.UserRoleOwner}, nil, router)
...
func TestGetNode_HidesNodesOutsideAuthorizedPeerSet(t *testing.T) {
	...
	}, fakePeers{{Key: "handle-a", Name: "visible", UserID: "user-a"}}, nil, nil, nil, nil, nil, nil, nil, nil, scanPermissions{role: types.UserRoleOwner}, nil, router)
...
func TestMemberPortalCanRenameAndRevokeOnlyOwnDevice(t *testing.T) {
	...
	router := mux.NewRouter()
	RegisterEndpoints(fakeNodes{"handle-a": {Handle: "handle-a"}, "handle-b": {Handle: "handle-b"}}, ownedDevices, ownedDevices, nil, nil, nil, nil, nil, nil, nil, scanPermissions{role: types.UserRoleUser}, nil, router)
...
func TestMemberAccessExplainsCompiledDestinationWithRuleAndGroup(t *testing.T) {
	...
	router := mux.NewRouter()
	RegisterEndpoints(fakeNodes{"handle-a": {Handle: "handle-a"}, "handle-b": {Handle: "handle-b"}}, devices, fakeOwnDevices{peers: devices}, nil, policy, nil, nil, nil, nil, nil, scanPermissions{role: types.UserRoleUser}, nil, router)
...
func TestMemberEnrollmentIssuesShortLivedSingleUseKey(t *testing.T) {
	...
	router := mux.NewRouter()
	RegisterEndpoints(fakeNodes{}, devices, enrollmentWriter{fakeOwnDevices: fakeOwnDevices{peers: devices}, got: got}, nil, nil, nil, nil, nil, nil, nil, scanPermissions{role: types.UserRoleUser}, nil, router)
...
func TestNodePathsIncludesReportedByteCounts(t *testing.T) {
	...
	router := mux.NewRouter()
	RegisterEndpoints(nodes, fakePeers{{ID: "peer-a", Key: "handle-a", Name: "mine", UserID: "user-a"}}, nil, nil, nil, nil, nil, nil, nil, nil, scanPermissions{role: types.UserRoleAdmin}, nil, router)
...
func TestPolicySchemaNeedsNoPolicyStore(t *testing.T) {
	router := mux.NewRouter()
	RegisterEndpoints(fakeNodes{}, fakePeers{}, nil, nil, nil, nil, nil, nil, nil, nil, scanPermissions{role: types.UserRoleOwner}, nil, router)
...
func TestPolicyWritePushesAnAccountWideUpdate(t *testing.T) {
	...
	router := mux.NewRouter()
	RegisterEndpoints(fakeNodes{}, fakePeers{}, nil, nil, scanPolicy{}, nil, nil, nil, nil, accounts, scanPermissions{role: types.UserRoleOwner}, nil, router)
...
func TestPolicyRollbackPushesAnAccountWideUpdate(t *testing.T) {
	accounts := &spyAccountUpdater{}
	router := mux.NewRouter()
	RegisterEndpoints(fakeNodes{}, fakePeers{}, nil, nil, rollbackablePolicy{}, nil, nil, nil, nil, accounts, scanPermissions{role: types.UserRoleOwner}, nil, router)
...
func TestPolicyWriteWithNoAccountUpdaterDoesNotPanic(t *testing.T) {
	router := mux.NewRouter()
	RegisterEndpoints(fakeNodes{}, fakePeers{}, nil, nil, scanPolicy{}, nil, nil, nil, nil, nil, scanPermissions{role: types.UserRoleOwner}, nil, router)
...
func TestPolicyPreviewCompilesFiftyNodesUnderOneSecond(t *testing.T) {
	...
	router := mux.NewRouter()
	RegisterEndpoints(nodes, peers, nil, nil, scanPolicy{}, nil, nil, nil, nil, nil, scanPermissions{role: types.UserRoleOwner}, nil, router)
...
func TestBedrockEnforcingStaleacknowledgmentReturnsConflict(t *testing.T) {
	...
	require.NoError(t, err)
	router := mux.NewRouter()
	RegisterEndpoints(fakeNodes{"node-a": {Handle: "node-a"}, "node-b": {Handle: "node-b"}}, fakePeers{{Key: "node-a", UserID: "user-a"}, {Key: "node-b", UserID: "user-a"}}, nil, nil, nil, nil, nil, store, nil, nil, scanPermissions{role: types.UserRoleOwner}, nil, router)
...
func TestBedrockOfflineCeremonyCoversEnrollmentBeforeEnforcing(t *testing.T) {
	...
	router := mux.NewRouter()
	RegisterEndpoints(fakeNodes{handle: {Handle: handle, PublicKey: identityKey, KemPublicKey: kem}}, fakePeers{{Key: handle, UserID: "user-a"}}, nil, auditLog, nil, nil, nil, configuration, chain, nil, scanPermissions{role: types.UserRoleOwner}, nil, router)
...
func TestAuditListReportsAnchorContradiction(t *testing.T) {
	...
	router := mux.NewRouter()
	RegisterEndpoints(fakeNodes{}, fakePeers{}, nil, auditLog, nil, nil, nil, nil, chain, nil, scanPermissions{role: types.UserRoleOwner}, nil, router)
...
func TestAllRegisteredResponsesExcludeSecretSentinels(t *testing.T) {
	...
	for _, role := range []types.UserRole{types.UserRoleOwner, types.UserRoleAdmin, types.UserRoleNetworkAdmin, types.UserRoleAuditor, types.UserRoleUser} {
		router := mux.NewRouter()
		RegisterEndpoints(fakeNodes{"handle-a": {Handle: "handle-a", PublicKey: []byte(secrets[0]), KemPublicKey: []byte(secrets[1])}}, fakePeers{{ID: "peer-a", Key: "handle-a", Name: "node", UserID: "user-a", SSHKey: secrets[2]}}, nil, scanAudit{}, scanPolicy{}, scanRelays{}, scanTurns{}, bedrockStore, nil, nil, scanPermissions{role: role}, nil, router)
		require.NoError(t, router.Walk(func(route *mux.Route, _ *mux.Router, _ []*mux.Route) error {
			template, err := route.GetPathTemplate()
...
func TestAuditExportRequiresAndStreamsRequestedFormat(t *testing.T) {
	...
	router := mux.NewRouter()
	RegisterEndpoints(fakeNodes{}, fakePeers{}, nil, exportAudit{entries: entries}, nil, nil, nil, nil, nil, nil, scanPermissions{role: types.UserRoleAdmin}, nil, router)
...
func TestRelayResponsesUseContractFieldsAndHealth(t *testing.T) {
	...
	router := mux.NewRouter()
	RegisterEndpoints(fakeNodes{}, fakePeers{}, nil, nil, nil, relays, nil, nil, nil, nil, scanPermissions{role: types.UserRoleOwner}, nil, router)
...
func TestTurnServerCRUD(t *testing.T) {
	...
	router := mux.NewRouter()
	RegisterEndpoints(fakeNodes{}, fakePeers{}, nil, nil, nil, nil, store, nil, nil, nil, scanPermissions{role: types.UserRoleOwner}, nil, router)
	user := auth.UserAuth{AccountId: "account-a", UserId: "admin"}
