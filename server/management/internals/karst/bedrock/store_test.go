...
// covered node needs no acknowledgement at all.
func TestACoveredNodeNeedsNoAcknowledgement(t *testing.T) {
	store := newStore(t)
	ctx := context.Background()
	at := int64(1000)
	keys := enrolled()
	...
	// Everyone covered: an empty acknowledgement is correct and sufficient.
