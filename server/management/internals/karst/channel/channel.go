...
// identity is ML-DSA-87 plus a static ML-KEM-768 key; nothing here may be used
...
// Version is the envelope format version *and* the cipher suite: the
// construction in ADR-0011, with the algorithms `suite.go` lists for it.
//
// One number for both, deliberately — see the package comment in suite.go. An
// algorithm cannot change without this changing, which is the property that
// went missing when ADR-0015 moved this channel to ML-DSA-87 and nothing
// objected.
const Version = 1
...
// Verifier checks an ML-DSA-87 signature. It is an interface because Go has no
...
// IdentityLookup resolves a node_id to its stored ML-DSA-87 public key.
...
type ServerPins struct {
	...
	VerifyKey []byte // ML-DSA-87 verification key, 2592 B
}
...
func Initiate(hello *proto.ChannelHello, pins ServerPins, verifier Verifier, nodeID []byte, signer Signer, presentIdentity bool) (*proto.ChannelInit, *Channel, error) {
	// A server that offers something else is refused with a reason. `SuiteFor`
	// distinguishes "never heard of it" from "known and not built here", which
	// is the difference between an operator suspecting corruption and an
	// operator knowing they need a different build.
	if _, err := SuiteFor(hello.GetVersion()); err != nil {
		return nil, nil, err
	}
...
func (s *StaticKey) Accept(p *Pending, init *proto.ChannelInit, lookup IdentityLookup, verifier Verifier) (*Channel, []byte, error) {
	if _, err := SuiteFor(init.GetVersion()); err != nil {
		return nil, nil, err
	}
