...
// The rest of the channel tests stand Ed25519 in for ML-DSA-65 to stay
...
func TestHandshakeSizes(t *testing.T) {
	...
	checks := []struct {
		...
		{"init.identity_pk", len(init.GetIdentityPk()), 1952},
		{"init.signature", len(init.GetSignature()), 3309},
		{"hello.signature", len(hello.GetSignature()), 3309},
	}
