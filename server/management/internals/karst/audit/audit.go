...
// Delivery is the durable outbox entry for one audit entry and one configured
// sink. It is deliberately separate from Entry: writing an audit event must
// commit even while a receiver is down, and delivery can then retry without
// changing the hash chain.
type Delivery struct {
	AccountID   string    `gorm:"primaryKey;size:64"`
	SinkID      string    `gorm:"primaryKey"`
...
func (Delivery) TableName() string { return "karst_audit_deliveries" }
...
func New(db *gorm.DB) (*Log, error) {
	...
	if err := db.AutoMigrate(&Entry{}, &Sink{}, &Delivery{}); err != nil {
...
func (l *Log) Append(ctx context.Context, actor, action, target, detail string) (*Entry, error) {
	...
	if err := l.enqueueDeliveries(ctx, written); err != nil {
		// The event is already durable. Returning it preserves the audit record
		// and lets the caller's normal mutation succeed; the retry worker can
		// only deliver rows that made it into the outbox, so log this loudly.
		return &written, fmt.Errorf("audit: enqueue delivery: %w", err)
	}
	return &written, nil
}

func (l *Log) enqueueDeliveries(ctx context.Context, entry Entry) error {
	accountID, err := accountFromContext(ctx)
	if err != nil {
		// Package-level callers (including chain tests) may intentionally create
		// an unscoped log. Such entries have no account-owned sink to deliver to.
	...
	var sinks []Sink
	if err := l.db.WithContext(ctx).Where("account_id = ?", accountID).Find(&sinks).Error; err != nil {
		return fmt.Errorf("list sinks: %w", err)
	}
	for _, sink := range sinks {
		delivery := Delivery{AccountID: accountID, SinkID: sink.ID, Sequence: entry.Seq, NextAttempt: time.Now().UTC()}
		if err := l.db.WithContext(ctx).Clauses(clause.OnConflict{DoNothing: true}).Create(&delivery).Error; err != nil {
			return fmt.Errorf("queue sink %s: %w", sink.ID, err)
		}
	}
	return nil
}
