...
// IdentityKeySize is an ML-DSA-65 public key. `karstd` refuses any other
...
type Entry struct {
	...
	// IdentityKey is the relay's ML-DSA-65 public key, base64.
	//
...
func (e Entry) compile() (*proto.KarstRelay, error) {
	...
	if len(key) != IdentityKeySize {
		return nil, fmt.Errorf("identity_key is %d bytes, want %d (an ML-DSA-65 public key, as `karst-relay pubkey` prints)",
