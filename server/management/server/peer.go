...
func (am *DefaultAccountManager) PortalPeers(ctx context.Context, accountID string) ([]*nbpeer.Peer, error) {
...
// RenameOwnPeer is the portal-only counterpart to UpdatePeer. It changes only
// the presentation name and reuses the normal DNS-label and netmap update
// mechanics, while keeping Members out of the broader peers:update module.
func (am *DefaultAccountManager) RenameOwnPeer(ctx context.Context, accountID, userID, peerID, name string) (*nbpeer.Peer, error) {
	if strings.TrimSpace(name) == "" {
		return nil, status.Errorf(status.InvalidArgument, "peer name cannot be empty")
	}
	var updated *nbpeer.Peer
	var dnsDomain string
	err := am.Store.ExecuteInTransaction(ctx, func(transaction store.Store) error {
		...
		if peer.ProxyMeta.Embedded {
			return status.Errorf(status.PreconditionFailed, "not allowed to rename peer")
		}
		settings, err := transaction.GetAccountSettings(ctx, store.LockingStrengthNone, accountID)
		if err != nil {
			return err
		...
		if err == nil {
			if existing, lookupErr := transaction.GetPeerIdByLabel(ctx, store.LockingStrengthNone, accountID, label); lookupErr == nil && existing != peer.ID {
				label = ""
			}
		} else {
			label = ""
		...
		if label == "" {
			label, err = getPeerIPDNSLabel(peer.IP, name)
			if err != nil {
				return fmt.Errorf("failed to get free DNS label: %w", err)
			}
		}
		peer.Name, peer.DNSLabel = name, label
		if err = transaction.IncrementNetworkSerial(ctx, accountID); err != nil {
			return fmt.Errorf("failed to increment network serial: %w", err)
		}
	...
	am.StoreEvent(ctx, userID, updated.ID, accountID, activity.PeerRenamed, updated.EventMeta(dnsDomain))
	affected := am.resolveAffectedPeersForPeerChanges(ctx, am.Store, accountID, []string{updated.ID})
	affected = append(affected, updated.ID)
	if err = am.networkMapController.OnPeersUpdated(ctx, accountID, []string{updated.ID}, affected); err != nil {
		return nil, fmt.Errorf("notify network map controller of peer update: %w", err)
	}
	return updated, nil
}
...
// DeleteOwnPeer removes a regular user's own peer without granting that user
// the general peers:delete permission. The ownership test lives in the same
// transaction as deletion: checking it only in an HTTP handler would leave a
// time-of-check/time-of-use gap for a concurrent reassignment.
func (am *DefaultAccountManager) DeleteOwnPeer(ctx context.Context, accountID, peerID, userID string) error {
	change := affectedpeers.Change{ChangedPeerIDs: []string{peerID}}
	settings, eventsToStore, snap, err := am.deleteOwnPeerInTransaction(ctx, accountID, userID, peerID, change)
	if err != nil {
		return err
	}
	for _, storeEvent := range eventsToStore {
	...
	if err = am.networkMapController.OnPeersDeleted(ctx, accountID, []string{peerID}, snap.Expand(ctx, accountID, change)); err != nil {
		log.WithContext(ctx).Errorf("failed to delete peer %s from network map: %v", peerID, err)
	}
	return nil
}

func (am *DefaultAccountManager) deleteOwnPeerInTransaction(ctx context.Context, accountID, userID, peerID string, change affectedpeers.Change) (*types.Settings, []func(), *affectedpeers.Snapshot, error) {
	var settings *types.Settings
	var eventsToStore []func()
