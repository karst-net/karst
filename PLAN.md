...
- **Re-baselined 2026-08-27 from the use-case implementation review.** The
  ...
  Karst administrative API, console, portal, role-limited auditor view,
  relay registry, policy lifecycle, and self-service one-time device keys now
  exist in the tree. Treat the detailed Phase 5 notes' original "absent"
  inventories as historical, not current state.
- **Bedrock's end-to-end workflow is complete and exercised across the real
  boundary**: root bootstrap, request export, offline signing, response import,
  signed-log distribution, and enforcing client rejection of an uncovered
...
- **Audit operations are complete for Phase 5**: the console/API agree on
...
- **Do not claim managed subnet routers or exit nodes in Phase 5.** Generic
  prefix routing is not a managed exit-node feature: route distribution,
  gateway lifecycle, client selection, forwarding controls, and ACL/node
  attributes remain Phase 6 work.
- **Exit:** a non-expert admin can install the server, connect three nodes
  across **Linux plus one completed non-Linux client platform** and two NATs,
  write an ACL, enable network lock through the full offline-signing workflow,
...
5. ~~**Name collision**~~ — **resolved 2026-08-09,
   ...
   public commit.
6. **CNSA 2.0 mandate?** — **reopened and answered YES on 2026-08-25.**
   ~~Resolved 2026-08-09 as: no target customers identified and no compliance
   date.~~ That answer is void. See
   [ADR-0015](docs/adr/0015-cnsa-2-0-as-a-mandate.md).

   Nearly every cryptographic decision in this document descends from the old
   ...
   **Resolved 2026-08-25: ML-DSA-87** (ADR-0015 Option A). What follows is the
   reasoning that led there.

   ~~One question is open: **what Bedrock's offline root becomes.**~~ Verified
   against NSA's CNSA 2.0 FAQ v2.1 on 2026-08-25 — SLH-DSA is not merely absent
   from the suite, it is **explicitly excluded and "not approved for any use in
   NSS"**, and NSA does not plan to add future NIST PQC standards. So the root
   changes regardless; today's key is Category 3 and the suite is Category 5
   throughout.

   Both candidates are compliant and the trade is narrow: **ML-DSA-87** is
   stateless and simple and loses the assumption diversity ADR-0001 and
   ADR-0014 exist to provide, while **LMS** is hash-based and keeps it at the
   cost of stateful key handling — which collides with §4.5's paper-backup
   posture, though `k`-of-`n` means independent keys rather than shared state.
   ADR-0015 §"Bedrock's offline root" states the trade in full. **Option A was
   taken**: ML-DSA-87, accepting the loss of assumption diversity rather than
   the operational hazard of stateful keys on offline `k`-of-`n` media. The
   consequence is recorded rather than glossed — a lattice break now takes the
   whole Bedrock hierarchy, recovery path included, and ADR-0014 carries a
   supersession notice saying so.

   Worth knowing while deciding: **the exclusion appears to be policy rather
   than cryptanalysis**, and Germany's BSI recommends SLH-DSA for exactly the
   reason ADR-0001 chose it. Karst's original design agrees with BSI and
   conflicts with NSA.

**All open questions are closed.** New ones belong here as they arise.
