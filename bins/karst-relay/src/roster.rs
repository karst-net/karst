...
//!
//! An entry names an ML-DSA-87 public key and nothing else. The node id and
...
pub enum Error {
    ...
    Syntax(String),
    /// An entry's key is not a 2592-byte ML-DSA-87 public key.
    BadKey(String),
...
#[derive(Debug, Deserialize)]
struct ClientRow {
    /// Standard base64 of the 2592-byte ML-DSA-87 identity key.
    identity_pk: String,
...
impl FileRoster {
    ...
    /// is not a 2592-byte ML-DSA-87 public key, [`Error::Duplicate`] for two
...
/// A syntactically valid ML-DSA-87 public key whose private half is discarded
