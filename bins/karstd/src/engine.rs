...
pub struct Stats {
    ...
    pub malformed: u64,
    /// Peer head claims that agreed with this node's verified chain
    /// (`bedrock-v1.md` §5 layer 3).
    pub bedrock_head_agreed: u64,
    /// **Proof of equivocation**: a peer reported a different log at a sequence
    /// this node also holds, which means the coordination server served two
    /// histories.
    ///
    /// Counted rather than fatal on purpose. Both nodes verified their peer
    /// against a valid chain, so tearing the session down would be a
    /// self-inflicted outage on the network an operator needs in order to
    /// investigate. **Any value above zero is an incident**, and it is the one
    /// counter here that means something has gone wrong with the server rather
    /// than with the network.
    pub bedrock_equivocation: u64,
...
struct PeerSlot {
    ...
    /// The Bedrock head this node last claimed to this peer, empty for none.
    ///
    /// Comparing against the current head means the claim goes out once, and
    /// again only if this node's log has actually advanced — which is a claim
    /// worth making, since the peer's copy is then behind. It also means the
    /// send is driven by *state* rather than by catching an event, which is
    /// what the first version got wrong: it hooked `Action::Established`, and
    /// that action fires only for the handshake initiator. A responder is
    /// promoted on the transport fast path and produces no action at all, so
    /// exactly half of every exchange was silently missing.
    claimed_head: Mutex<Vec<u8>>,
...
pub struct Engine {
    ...
    /// passed in per call because the comparison happens on the inbound
    /// datapath, where there is no other route to it.
    bedrock: RwLock<Option<Arc<crate::bedrock::Log>>>,
...
impl Engine {
    ...
    pub fn reconfigure(&self, config: &Arc<Config>) -> Reconfigured {
        ...
        for (index, peer) in previous.config.peers.iter().enumerate() {
            if let Some(slot) = previous.peers.get(index) {
                existing.insert(
                    peer_id_hint(&peer.public.kem_pk.to_bytes()),
        ...
        for peer in &config.peers {
            let hint = peer_id_hint(&peer.public.kem_pk.to_bytes());
    ...
    pub fn poll(&self, now_ms: u64, seed: impl Fn() -> [u8; 32]) -> Output {
        ...
        for index in 0..roster.peers.len() {
            ...
            let actions = roster
                ...
                .unwrap_or_default();
            self.apply(&roster, index, actions, now_ms, &mut out);
            self.claim_head_if_due(&roster, index, now_ms, &mut out);
        }
        out
    }
    ...
    pub fn outbound(&self, packet: &[u8], now_ms: u64) -> Output {
        ...
        self.send_sealed(&roster, peer, via, packet, now_ms, &mut out);
        out
    }

    /// Seal a payload to a peer and fragment it onto the wire.
    ///
    ...
    /// Publish the verified Bedrock log, so inbound head claims can be checked.
    pub fn set_bedrock(&self, log: Arc<crate::bedrock::Log>) {
        if let Ok(mut slot) = self.bedrock.write() {
            *slot = Some(log);
        }
    }
    ...
    ///
    /// Driven from [`Self::poll`] and gated on state rather than fired from an
    /// event, because there is no single event both roles reach.
    /// `Action::Established` is produced for the handshake *initiator* only; a
    /// responder adopts its keys on the transport fast path, which emits no
    /// action. Hooking that action sent exactly half of every exchange, and
    /// `tests/equivocation.rs` is what found it.
    ///
    /// Comparing the current head against the last one claimed makes this
    /// naturally once-per-head rather than once-per-session: no repetition
    /// while nothing changes, and a fresh claim when this node's log advances
    /// and the peer's copy is therefore behind.
    fn claim_head_if_due(&self, roster: &Roster, peer: PeerIndex, now_ms: u64, out: &mut Output) {
        ...
        let head = {
            let Ok(guard) = self.bedrock.read() else {
                return;
            };
            match guard.as_ref() {
                Some(log) if log.is_present() => log.head().to_vec(),
                _ => return,
            }
        };
        {
            let mut claimed = Self::lock(&slot.claimed_head);
            if *claimed == head {
                return;
            }
            claimed.clone_from(&head);
        }
        self.send_head_claim(roster, peer, now_ms, out);
    }

    /// Build and send the claim. Callers gate on whether it is due.
    fn send_head_claim(&self, roster: &Roster, peer: PeerIndex, now_ms: u64, out: &mut Output) {
        let claim = {
            let Ok(slot) = self.bedrock.read() else {
                return;
            };
            ...
            crate::bedrock::encode_head_claim(log.head(), log.verified_seq())
        };
        let Some(via) = self.via(roster, peer) else {
            return;
        };
        self.send_sealed(roster, peer, via, &claim, now_ms, out);
    }

    /// Compare a peer's head claim against this node's chain.
    ///
    /// Returns `true` when the payload was a claim and has been handled, so the
    /// caller knows not to treat it as a tunnelled packet.
    ///
    /// **A divergence does not tear the session down.** Both nodes verified
    /// their peer against a valid chain, and the fault is the server's; cutting
    /// the link would be a self-inflicted outage on exactly the network an
    /// operator needs in order to investigate. The correct response is a loud,
    /// counted alarm to a human — which is why this raises a counter and a log
    /// line and does nothing else.
    fn on_head_claim(&self, peer: PeerIndex, payload: &[u8]) -> bool {
        let Some((hash, seq)) = crate::bedrock::decode_head_claim(payload) else {
            return false;
        };
        let Ok(slot) = self.bedrock.read() else {
            return true;
        ...
        match log.compare_head(&hash, seq) {
            crate::bedrock::HeadComparison::Agrees => {
                self.stats
                    .bedrock_head_agreed
                    .fetch_add(1, Ordering::Relaxed);
            }
            crate::bedrock::HeadComparison::Diverges { seq } => {
                self.stats
            ...
            | crate::bedrock::HeadComparison::Unknown => {}
        }
        true
    }
    ...
    fn accept_relayed_handshake(
        ...
        let actions = roster
            ...
            .get(peer)
            .map(|p| {
                Self::lock(&p.session).adopt_responder(
                    msg,
                    &pending.confirm(),
                    &msg2,
                    now_ms,
    ...
    fn deliver_to_host(
        ...
        // Karst inner control frames are multiplexed ahead of the IP path.
        // The marker byte is zero, which is not a legal IP version, so this
        // cannot shadow a tunnelled packet — and it is inside the AEAD, unlike
        // the transport's outer type byte.
...
fn short_hint(pk: &KemPublicKey) -> String {
    let hint = peer_id_hint(&pk.to_bytes());
