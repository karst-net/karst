...
/// Most bytes buffered from the client while the tunnel is not draining.
///
...
/// that a fast local writer cannot spend the daemon's memory.
const MAX_BUFFERED: usize = 256 * 1024;

/// One read from the attached workload.
///
/// 4 KiB was three tunnel MTUs, which made the relay's own read the smallest
...
const ACTIVE_POLL: Duration = Duration::from_micros(200);
...
fn proxy(mut client: TcpStream, stack: &Userspace, shutdown: &Shutdown) -> io::Result<()> {
    ...
    // moment the client's read hit EOF, which tore down both halves at once and
    ...
    while !shutdown.requested() {
        // **Sleep only when a pass moved nothing.** The loop below is the whole
        ...
        if !client_done {
            match if from_client.len() >= MAX_BUFFERED {
                Err(io::Error::from(io::ErrorKind::WouldBlock))
            } else {
                client.read(&mut buf)
            } {
                // The client will send no more. Its *reply* may still be on the
                // way, so this closes one direction and keeps relaying the
                // other.
                Ok(0) => client_done = true,
                // Read only while there is room. Without this a client that
                // writes faster than the tunnel drains grows this buffer
                // without limit — the local process then chooses how much of
                // the daemon's memory to consume, which is the same mistake as
                // the unbounded wait above in a different shape.
                Ok(n) => {
                    from_client.extend_from_slice(buf.get(..n).unwrap_or_default());
                    moved = true;
                }
                Err(e) if e.kind() == io::ErrorKind::WouldBlock => {}
                Err(e) => return Err(e),
        ...
        // The FIN goes out only once everything the client wrote has been
        // handed to the stack; closing with bytes still buffered here would
        // truncate the request that the reply is an answer to.
        if client_done && from_client.is_empty() && !sent_fin {
            stack.tcp_close(tunnel);
            sent_fin = true;
        }
