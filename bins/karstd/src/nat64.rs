...
/// PREF64 router solicitation, where the platform cannot do it.
///
/// `RouterSocket` is an `ICMPv6` raw socket built on Linux interfaces, and
/// the interface enumeration below reads `/proc/net/if_inet6`. Neither exists
/// on macOS, and the honest answer there is the same one an unprivileged Linux
/// host gives: **this mechanism is unavailable, try the other one**. RFC 7050
/// discovery through `ipv4only.arpa` is DNS and runs everywhere, so a host on a
/// NAT64 network still finds its prefix — it just takes a query rather than a
/// solicitation.
///
/// This is a real gap, not a papered-over one: a network that advertises
/// PREF64 in a router advertisement and does *not* serve `ipv4only.arpa` will
/// leave a macOS node without a prefix, and it will say so on startup. Closing
/// it needs a `PF_ROUTE`/`ICMPv6` port, which is Phase 7 work alongside the
/// rest of the in-process networking — `plans/phase-5/06-macos-client.md` §2.
