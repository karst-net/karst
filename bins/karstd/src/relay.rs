...
/// an ML-DSA-87 signature, and short enough that a relay which has silently
...
mod tests {
    ...
    fn relay() -> Relay {
        let identity_key = vec![0x44; 2592];
    ...
    fn control_handle_becomes_the_raw_ponor_node_id() {
        let node_handle = karst_control_client::handle(&[0x11; 2592]);
        let relay = relay();
        let mut session =
