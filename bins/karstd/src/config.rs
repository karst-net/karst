...
pub struct File {
    ...
    /// Local DNS listener and host-integration policy. Resolver records and
...
/// DNS settings after TOML parsing, shared by static and netmap configurations.
#[derive(Debug, Clone)]
...
impl From<DnsSection> for DnsSettings {
    fn from(section: DnsSection) -> Self {
        Self {
            enabled: section.enabled,
            stub_address: section
                .stub_address
...
impl Default for DnsSettings {
    fn default() -> Self {
        DnsSection::default().into()
    }
}

/// Userspace DNS lives exclusively inside the encrypted overlay. It cannot be
/// a host resolver, so reporting any host integration there would be both
/// misleading and unsafe.
fn dns_for_network_mode(mut dns: DnsSettings, mode: NetworkMode) -> DnsSettings {
    if mode == NetworkMode::Userspace {
        dns.host_integration = HostIntegration::None;
    }
    dns
}
...
pub struct ControlSection {
    ...
    /// The lowest control-channel protocol version this node will accept, and
    /// so the weakest cipher suite — ADR-0015 item 4. Absent means 1.
    ///
    /// A **floor**, like `bedrock_mode`: the server states its version and this
    /// node refuses anything below. That is what stops a compromised or
    ...
    /// `advisory`, or `enforcing`. Absent means `off`.
    ///
    /// A **floor**, not a setting. The node enforces at the stronger of this
    /// and whatever the server advertises, so an operator who writes
    /// `enforcing` here cannot be talked down by a compromised coordination
    /// server — which is the whole reason Bedrock exists, and would be undone
    /// by a mode the server alone controlled. Leaving it unset lets the console
    /// drive enforcement, which is the expected deployment.
    pub bedrock_mode: Option<String>,
...
pub struct Config {
    ...
    pub network_mode: NetworkMode,
    /// Local `KarstDNS` listener policy.
    pub dns: DnsSettings,
    /// Authenticated DNS policy from the currently assembled netmap. Empty for
    /// a static roster, which has no control-plane DNS source.
    pub netmap_dns: crate::netmap::DNSConfig,
...
impl Config {
    ...
    fn from_file(file: File, config_path: &Path) -> Result<Self, ConfigError> {
        ...
        Ok(Self {
            ...
            dns: dns_for_network_mode(file.dns.into(), file.node.network_mode),
    ...
    pub fn from_netmap(local: LocalSettings, netmap: &Netmap) -> Result<Self, ConfigError> {
        Self::from_netmap_enforced(local, netmap, &std::collections::BTreeMap::new())
    }

    /// As [`Config::from_netmap`], with Bedrock exclusions applied.
    ///
    /// `excluded` maps a peer handle to the reason it is not covered. Excluded
    /// peers are dropped from the datapath configuration entirely and recorded
    /// as skipped, which is the same treatment an unparseable peer gets — and
    /// for a related reason. **A peer that is not in the configuration cannot
    /// be handshaked with**, so enforcement happens by construction rather than
    /// by a check somewhere in the session path that could be forgotten.
    ///
    /// # Errors
    ///
    ...
    pub fn from_netmap_enforced(
        ...
        for entry in netmap.peers() {
            let handle = String::from_utf8_lossy(&entry.node_id).into_owned();
            if let Some(reason) = excluded.get(&handle) {
                skipped.push(SkippedPeer {
                    handle,
                    dns_name: entry.dns_name.clone(),
                    reason: format!("bedrock: {reason}"),
                });
                continue;
            }
            let index = peers.len();
        ...
        Ok(Self {
            ...
            dns: dns_for_network_mode(local.dns, local.network_mode),
            netmap_dns: netmap.dns_config.clone(),
...
impl fmt::Debug for LocalSettings {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("LocalSettings")
            ...
            .field("interface", &self.interface)
            .field("network_mode", &self.network_mode)
            .field("dns_enabled", &self.dns.enabled)
            .field("userspace_socks5_listen", &self.userspace_socks5_listen)
            .field("userspace_publish", &self.userspace_publish)
            .field("nat64", &self.nat64)
            .finish_non_exhaustive()
...
/// Read only the interface name and `[dns]` table, ignoring everything else.
///
/// `karst dns revert` runs after the daemon — and the interface and control
/// session it validated its full configuration against — may already be gone
/// (`ExecStopPost=` fires once the main process has exited). Routing it
/// through [`Config::load`] would refuse a server-managed node's own file
/// (which names `[control]`, not `[[peer]]`) and would fail entirely if the
/// private key file is unreadable for any reason. Revert needs neither the
/// roster nor the keys, only the two settings that named the host change.
///
/// # Errors
/// [`ConfigError`] if the file cannot be read or parsed as TOML.
pub fn load_dns_settings(path: &Path) -> Result<(DnsSettings, String), ConfigError> {
    #[derive(serde::Deserialize)]
    struct Partial {
        node: PartialNode,
        #[serde(default)]
    ...
    let parsed: Partial = toml::from_str(&text).map_err(|e| ConfigError::Parse {
        path: path.to_owned(),
        message: e.to_string(),
    })?;
    Ok((
        dns_for_network_mode(parsed.dns.into(), parsed.node.network_mode),
...
impl Peer {
    ...
    fn from_netmap(
        ...
        // Length decides the parameter set — see `from_section`.
        let kem_pk =
            KemPublicKey::from_bytes_by_length(&entry.kem_public_key).ok_or_else(|| {
    ...
    fn from_section(
        ...
        let kem_pk = KemPublicKey::from_bytes_by_length(&kem_bytes).ok_or_else(|| {
            ConfigError::InvalidKey {
                peer: name.clone(),
                field: "kem_public_key",
            }
...
"#
    ...
    /// operator who has truncated a paste can see what the two legal answers
    /// were.
    #[test]
    fn a_peer_key_of_neither_length_is_refused_naming_both() {
        let dir = Scratch::new("cfg-badlen");
    ...
    #[test]
    fn parses_explicit_dns_listener_policy() {
        let dir = Scratch::new("cfg-dns");
        let path = roster(dir.path(), "");
        let source = std::fs::read_to_string(&path).expect("read roster");
        std::fs::write(
            &path,
            format!(
                "{source}\n[dns]\nenabled = true\nstub_address = \"127.0.0.2:5353\"\naccept_netmap_config = false\nupstream = [\"1.1.1.1:53\"]\nhost_integration = \"none\"\n"
            ),
        )
        .expect("write roster");
        ...
        assert_eq!(
            cfg.dns.stub_address,
            "127.0.0.2:5353".parse().expect("address")
        );
        assert!(!cfg.dns.accept_netmap_config);
        assert_eq!(
            cfg.dns.upstream,
    ...
    /// so it still works on a server-managed node's file, which names
    /// `[control]` and has no `[[peer]]` — and even one whose private key
    /// file has since been removed.
    #[test]
    fn dns_settings_load_from_a_file_load_would_refuse() {
        let dir = Scratch::new("cfg-dns-only");
...
identity_key_file = "missing-identity.key"
...
"#,
        ...
        let (dns, interface) = load_dns_settings(&path).expect("dns-only load");
        assert_eq!(interface, "karst7");
    ...
    fn dns_settings_default_when_the_table_is_absent() {
        ...
        let path = roster(dir.path(), "");
        let (dns, interface) = load_dns_settings(&path).expect("dns-only load");
        assert_eq!(interface, "karst0");
        assert_eq!(dns.host_integration, HostIntegration::Auto);
    }
