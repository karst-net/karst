...
async fn start_relay(tag: &str, nodes: &[&Identity]) -> Running {
    ...
    // is the ML-DSA-87 key pinned below.
...
/// upgrade, and the Ponor handshake with real ML-DSA-87 on both sides.
#[tokio::test]
async fn a_node_completes_a_ponor_handshake_with_a_real_relay() {
    ...
    // **the `Ok` is the assertion**: it means the relay's ML-DSA-87 signature
