...
| NET-08 | macOS managed-device detection | On a macOS test device, first confirm ordinary self-service enroll/disconnect/removal via `Karst.app`. Then install the sample profile (`sudo profiles install -type configuration -path dev.karst.packettunnel.example.mobileconfig`) on a separate device and reopen `Karst.app`'s menu. | Personal-mode device: no regression, Enroll/Re-enroll work as before. Profile-installed device: menu shows "VPN configuration managed by your organization"; System Settings shows the profile's removal control greyed out or password-gated. |
| NET-09 | macOS kill switch | With the managed profile active and the tunnel connected, force the engine to fail mid-session (kill the extension process or corrupt its `config.toml`/socket state), then attempt to reach the open internet from another app. | Zero packets escape the tunnel while the engine is down — validates `IncludeAllNetworks` is actually honored for this provider on the tested macOS version. |
| NET-10 | macOS reconnect and route-churn reassert | Toggle Wi-Fi off/on (or sleep/wake) while connected; separately, toggle an exit route active/inactive from another enrolled peer mid-session. | Tunnel automatically re-establishes via the on-demand rule; the exit-route change is reflected in routing within a few seconds, without requiring a full tunnel restart. |
