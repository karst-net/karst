...
import type { EnrollmentMetadata } from "@karst-net/ui";
import { accessToken, loadConfig, renewOnce, login } from "./auth";
...
async function request<T>(path: string, init?: RequestInit, retried = false): Promise<T> {
  ...
  const response = await fetch(base + path, { ...init, headers: { "content-type": "application/json", ...init?.headers, ...(token ? { authorization: `Bearer ${token}` } : {}) } });
  if (response.status === 401) {
    const config = await loadConfig();
    if (!retried && config.oidcAuthority && await renewOnce(config)) return request(path, init, true);
    if (config.oidcAuthority) login(config);
  }
  if (!response.ok) throw new Error((await response.json().catch(() => ({ message: response.statusText }))).message ?? "Request failed");
