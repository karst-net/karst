...
import { EmptyState, Observed, Enrollment } from "@karst-net/ui";
...
function App({ config, auth }: { config: AuthConfig; auth: AuthState }) { const [route, setRoute] = useState(current()); useEffect(() => { const handler = () => setRoute(current()); addEventListener("hashchange", handler); return () => removeEventListener("hashchange", handler); }, []); return <div className="shell"><a className="skip" href="#main">Skip to content</a><aside><h1>Karst</h1><p>Your network</p><nav aria-label="Primary">{routes.map(([key, label]) => <a key={key} href={`#/${key}`} aria-current={route === key ? "page" : undefined}>{label}</a>)}</nav></aside><main id="main">{auth === "authenticated" && <button onClick={() => logout(config)}>Log out</button>}{route === "devices" && <Devices />}{route === "download" && <Download />}{route === "access" && <AccessView />}{route === "sessions" && <Sessions />}</main></div>; }
const root = createRoot(document.getElementById("root")!);
async function boot() {
  const config = await loadConfig();
  const auth = await bootstrap(config);
  root.render(<StrictMode>{auth === "anonymous" ? <main><h1>Karst</h1><p>Sign in to connect your devices.</p><button onClick={() => login(config)}>Log in</button></main> : auth === "disabled" && !import.meta.env.DEV ? <main><h1>Karst</h1><p role="alert">Sign-in is not configured. Ask your administrator to configure the identity provider.</p></main> : <App config={config} auth={auth} />}</StrictMode>);
}
void boot().catch(() => root.render(<main><h1>Karst</h1><p role="alert">Sign-in failed. Reload to try again.</p></main>));
