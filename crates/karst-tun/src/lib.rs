...
//!
//! A TUN interface is a file descriptor that yields one bare IP packet per read
//! and injects one per write. The kernel routes to it like any other interface,
//! so `karstd` reads outbound packets here, encrypts them, and writes inbound
//! plaintext back.
//!
//! [`Tun`] is that interface on whichever platform this is built for — Linux's
//! `/dev/net/tun` in `IFF_TUN`/`IFF_NO_PI` mode, or macOS's `utun`. The two
//! implementations expose the same surface, and the differences between them
//! stop at the module boundary: macOS's four-byte address-family prefix is
//! added and removed inside [`macos`], and its lack of segmentation offload
//! shows up only as [`Tun::offload`] answering false. Nothing above this crate
//! is written twice.
//!
//! # Privileges
//!
//! Creating an interface needs `CAP_NET_ADMIN` on Linux and root on macOS, and
//! so does setting its MTU, address, or routes. The failure arrives at
//! different points: `/dev/net/tun` is usually world-accessible, so a Linux
//! process opens the device successfully and then fails at `TUNSETIFF`, while
...
// are easy to get wrong and impossible to see, so its tests run on the same
...
pub enum TunError {
    ...
    /// A system tool refused an operation — macOS only, where addressing and
    /// routing go through `ifconfig` and `route` rather than through a
    /// syscall. See [`macos`] for why.
    ///
    /// `detail` carries the tool's own message rather than a paraphrase: it is
    /// the more specific of the two, and an operator can search for it.
    Tool {
...
/// Every global-scope unicast address this host currently holds.
///
/// As the Linux path above, through `getifaddrs(3)`. It carries one documented
/// gap — tentative and deprecated IPv6 addresses are not filtered, because
/// `getifaddrs` does not report those flags; see [`sys_macos::local_addresses`]
/// for what that costs and why it is not a correctness problem.
///
/// # Errors
/// [`TunError::Netlink`] if the enumeration fails. The variant is shared with
/// the Linux path rather than duplicated: it means "the host's own network
/// configuration could not be read", which is the same fact on both.
#[cfg(target_os = "macos")]
pub fn local_addresses() -> Result<Vec<std::net::IpAddr>, TunError> {
    sys_macos::local_addresses().map_err(|source| TunError::Netlink {
        op: "getifaddrs",
        source,
    })
}
...
///
/// As the Linux path above, through `sysctl(NET_RT_DUMP)`.
///
/// # Errors
/// [`TunError::Netlink`] if the routing table cannot be read.
#[cfg(target_os = "macos")]
pub fn default_gateway() -> Result<Option<std::net::IpAddr>, TunError> {
    sys_macos::default_gateway().map_err(|source| TunError::Netlink {
        op: "sysctl(NET_RT_DUMP)",
        source,
    })
}
