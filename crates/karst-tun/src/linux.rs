...
impl Tun {
    ...
    /// Kernel interface index, for services such as systemd-resolved that
    /// scope their configuration to an interface rather than its name.
    ///
    /// The value is looked up when requested instead of cached: an interface
    /// may be deleted and recreated with the same name by an external manager.
    ///
    /// # Errors
    ///
    /// Returns [`TunError::Ioctl`] when the kernel rejects the interface
    /// lookup, including when the interface disappeared concurrently.
    pub fn ifindex(&self) -> Result<u32, TunError> {
        let ctl = sys::control_socket().map_err(|source| TunError::Ioctl {
            op: "socket(AF_INET)",
            source,
        })?;
        sys::interface_index(ctl.as_fd(), self.encoded_name()?).map_err(|source| TunError::Ioctl {
            op: "SIOCGIFINDEX",
            source,
        })
    }
    ...
    /// Add `addr/prefix_len` to this interface **without** displacing
    /// whatever address [`Tun::set_address`] already assigned it.
    ///
    /// # Why this exists separately from `set_address`
    ///
    /// `set_address` assigns through `SIOCSIFADDR`, the classic ioctl that
    /// *replaces* an interface's address — calling it a second time for a
    /// different address leaves only the second one behind, silently. The
    /// `KarstDNS` stub host address must coexist with the node's own overlay
    /// address on the same kernel TUN device, so it needs the same additive
    /// rtnetlink path `add_route` already uses for routes: `RTM_NEWADDR`
    /// adds an address alongside any others already present, rather than
    /// replacing them.
    ///
    /// # Errors
    /// [`TunError::Netlink`] if the kernel refuses the request.
    pub fn add_secondary_address(&self, addr: IpAddr, prefix_len: u8) -> Result<(), TunError> {
        ...
        let seq = self.next_seq();
        sys::new_address(nl.as_fd(), seq, addr, prefix_len, index).map_err(|source| {
            TunError::Netlink {
                op: "RTM_NEWADDR",
                source,
            }
