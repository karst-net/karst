...
//!
//! Linux only, and not merely because of the `ioctl`s: every assertion here
//! reads the kernel's own record through `/sys/class/net/`, and asks for an
//! interface by name. Neither has a macOS counterpart — `utun` interfaces are
//! named by the kernel and there is no sysfs — so the macOS device tests are a
//! separate file rather than a `cfg`-riddled version of this one. See
//! `tests/utun.rs`.
