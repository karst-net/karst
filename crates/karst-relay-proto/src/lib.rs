...
pub mod consts {
    ...
    /// largest legal frame ([`CLIENT_AUTH_LEN`], 4693 B), and it is enforced
    /// in addition to the exact per-type lengths, not instead of them.
    ///
    /// **Doubled from 4096 when ADR-0015 moved identities to ML-DSA-87.** This
    /// is the bound a relay allocates against before it has authenticated
    /// anybody, so Category 5 costs twice the pre-authentication memory per
    /// concurrent connection — a real cost of the mandate rather than a
    /// constant that happened to need bumping.
    pub const FRAME_PAYLOAD_MAX: usize = 8192;
    ...
    pub const IDENTITY_PK_LEN: usize = 2592;
...
mod tests {
    ...
    fn frame_lengths_match_the_spec() {
        ...
        assert_eq!(CLIENT_AUTH_LEN, 4693);
        assert_eq!(RELAY_AUTH_LEN, 4628);
    ...
    #[test]
    fn handshake_costs_under_ten_kilobytes() {
        ...
        //
        // **6 762 B before ADR-0015, 9 398 B after.** Moving identities to
        // ML-DSA-87 costs 39% on every relay handshake, which is what Category
        // 5 buys and what it charges.
        let total = 3 * FRAME_HEADER + RELAY_HELLO_LEN + CLIENT_AUTH_LEN + RELAY_AUTH_LEN;
        assert_eq!(total, 9398);
    }
}
