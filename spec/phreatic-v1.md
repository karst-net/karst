...
| ID | Name | KEM | DH | Signature | AEAD | Hash |
|---|---|---|---|---|---|---|
| `0x0001` | `KARST_1` | ML-KEM-768 | X25519 | ML-DSA-65 | ChaCha20-Poly1305 | SHA-512 |
| `0x0002` | `KARST_2` | ML-KEM-768 | X25519 | ML-DSA-65 | AES-256-GCM | SHA-512 |
| `0x0003` | `KARST_3` | ML-KEM-1024 | — | ML-DSA-87 | AES-256-GCM | SHA-384 |

Suite `0x0003` is the CNSA 2.0 profile and is **PQ-only** (no classical hybrid).
It is defined here so the wire format is general; it is a Phase 7 deliverable
and implementations MAY reject it in v1.
...
|---|---|---|---|
| Identity `I` | ML-DSA-65 | 1952 B | Signed by the Bedrock chain; establishes the node is authorised to exist |
| Static KEM `S` | ML-KEM-768 | 1184 B | Post-quantum authentication in the handshake |
...
(PLAN.md §4.5) and is listed for completeness.

The **peer identity hint** is

```
peer_id_hint = HASH("Karst peer-id v1" ‖ S_pk)[0..32]
```
...
### 6.5 Suite `0x0003` needs three fragments

The two-fragment property is specific to suites `0x0001`/`0x0002`. The CNSA 2.0
profile uses ML-KEM-1024 — 1568-byte encapsulation keys *and* ciphertexts — and
carries no X25519:

| Suite | HandshakeInit | HandshakeResponse | Fragments |
|---|---|---|---|
| `0x0001` / `0x0002` | 2378 | 2236 | **2** |
| `0x0003` | 3210 | 3164 | **3** |
...
operator enabling suite `0x0003` on a lossy link should expect more handshake
...
with the **suite hash** — HMAC-SHA-512 for suites `0x0001`/`0x0002`,
HMAC-SHA-384 for `0x0003` — and
...
identity as ML-DSA-65 plus a static ML-KEM-768 key. Writing §7 exposed a gap:
