...
over an ML-DSA-87 key published in the relay registry.
...
`identity_pk` is the node's ML-DSA-87 identity key — the same key that
...
A relay holds its **own** ML-DSA-87 keypair, distinct from any node identity.
...
sig_client = ML-DSA-87.Sign(identity_sk, ctx,
                 H("ponor-client-auth-v1" ‖ relay_id ‖ relay_random
                                          ‖ client_random ‖ peer_id ‖ role))

sig_relay  = ML-DSA-87.Sign(relay_identity_sk, ctx,
...
the smallest power of two above the largest legal frame (`ClientAuth`, 4693 B)
...
| `0x02` | `ClientAuth` | peer → relay | `version ‖ role ‖ peer_id ‖ client_random ‖ sig_client` | 4693 |
| `0x03` | `RelayAuth` | relay → peer | `version ‖ sig_relay` | 4628 |
...
arriving after that point comes from the ML-DSA-87 identity the netmap pinned.
...
a full ML-DSA-87 verification only when the lookup hits. That difference is
...
The decoy is any syntactically valid ML-DSA-87 public key with no corresponding
